...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # for i_layer in range(2, len(embed_dims)):
        #     layer = nn.LayerNorm((embed_dims[i_layer]))
    ...
    def forward(self, x):
        ...
        N, C, H, W = x.shape
        outs = []
        x = self.patch_embed(x) # 96
        # outs.append(x.view(N, H//4, W//4, self.embed_dims[0]).permute(0, 3, 1, 2).contiguous())
        x = self.layers[0](x) # 192
        outs.append(x.view(N, H//8, W//8, self.embed_dims[1]).permute(0, 3, 1, 2).contiguous())
        start_i = 1
        ...
        for i in range(start_i, len(self.layers)):
            ...
            if i in self.out:
                ...
                x_out = x.view(-1, H//2, W//2, self.embed_dims[i+1]).permute(0, 3, 1, 2).contiguous()
                outs.append(x_out)
...
if __name__ == "__main__":
    model = tiny_vit_21m_512(pretrained=True)
